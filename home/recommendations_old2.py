...
def generateRecommendation(request):
    ...
    books_df = pd.DataFrame(y,columns=['BookUid','Name','Image','Category','slug', 'price'])
    ...
    if request.user.is_authenticated:
        ...
        #For every user group in our subset
            ...
            print("\n\nRecommendation_df\n\n", recommendation_df.head())
            recommender=books_df.loc[books_df['BookUid'].isin(recommendation_df.head(4)['BookUid'].to_list())]
            print("\n\n\n\n\nFinal recommendations\n\n\n\n",recommender)
            # return recommender.to_dict('records')
            return recommendation_df['BookUid'].tolist()